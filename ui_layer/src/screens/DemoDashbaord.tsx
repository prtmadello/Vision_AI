...
const DemoDashboard = () => {
  ...
  useEffect(()=>{
    const fetchLiveEvent = async ()=>{
        await axios.get(`${import.meta.env.VITE_API}/alert`).then((res)=>{
            console.log(res)
            setEvent(res.data.alerts)
        }).catch((err)=>{
            console.log(err)
        })
    }
  ...
  chennai: {
    tambaram: {
      cards: [
        ...
        { title: "total incidents", paragraph: "Reported incidents today", value: event.length, icon: FaExclamationTriangle, iconStyle: "bg-quaternary text-white text-lg p-2 rounded-full !text-sm xxl:text-2xl" },
      ],
      graph: [
      ...
      incidents: event.map((item,index)=>{
        return(
            {   
                location: "Chennai", shop: "Tambaram", date: "10.10.2025 11:45:30 AM", thumbnail: item.message, behaviour: "Suspicious", confidence: item.detection_confidence, camera: "Entrance Cam 1"
            }
        )
      })
    //   [
...
const CustomerSection = () => {
  ...
  const incidentColumns = [
    { label: "Location", render: () => selectedLocation },
    ...
    return (
      <section className="foreground p-5 rounded-2xl shadow-minimal">
        <div className="space-y-5">
          ...
          <Line />
          <div className="">
            <DataTable data={currentData.incidents} columns={incidentColumns} minRows={2} pagination={true}/>
          </div>
        </div>
